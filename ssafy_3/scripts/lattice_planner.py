...
class latticePlanner:
    def __init__(self):
        ...
        self.is_lattice_pub = rospy.Publisher('is_lattice', Int8, queue_size=1)
        ...
        while not rospy.is_shutdown():
            # print(self.is_path, self.is_status, self.is_obj)
            try:
                if self.is_path and self.is_status and self.is_obj:
                    if self.checkObject(self.local_path, self.object_data):
                        ...
                        self.is_lattice_pub.publish(1)
                    else:
                        self.lattice_path_pub.publish(self.local_path)
                        self.is_lattice_pub.publish(0)
            except:
                pass
            rate.sleep()
    ...
    def checkObject(self, ref_path, object_data):
        ...
        for npc in object_data.npc_list:
            for path in ref_path.poses:
                dis = sqrt(pow(path.pose.position.x - npc.position.x, 2) + pow(
                    path.pose.position.y - npc.position.y, 2))
                if dis < 2.35:  # 자동차 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
                    is_crash = True
                    break

        for ped in object_data.pedestrian_list:
            for path in ref_path.poses:
                dis = sqrt(pow(path.pose.position.x - ped.position.x, 2) + pow(
                    path.pose.position.y - ped.position.y, 2))
                if dis < 2.35:  # 보행자 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
                    is_crash = True
                    break

        for obstacle in object_data.obstacle_list:
            for path in ref_path.poses:
                dis = sqrt(pow(path.pose.position.x - obstacle.position.x, 2) + pow(
                    path.pose.position.y - obstacle.position.y, 2))
                if dis < 2.35:  # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
                    is_crash = True
                    break
        return is_crash
    ...
    def collision_check(self, object_data, out_path):
        ...
        for npc in object_data.npc_list:
            ...
            for path_num in range(len(out_path)):
                for path_pos in out_path[path_num].poses:
                    dis = sqrt(pow(npc.position.x - path_pos.pose.position.x, 2) + pow(
                        npc.position.y - path_pos.pose.position.y, 2))
                    # if npc.unique_id == 7:
                    if dis < 1.5:
                        # print(npc)
                        # print('\n')
        ...
        for ped in object_data.pedestrian_list:
            for path_num in range(len(out_path)):
                for path_pos in out_path[path_num].poses:
                    dis = sqrt(pow(ped.position.x - path_pos.pose.position.x, 2) + pow(
                        ped.position.y - path_pos.pose.position.y, 2))
                    if dis < 1.5:
                        lane_weight[path_num] = lane_weight[path_num] + 100
    ...
    def latticePlanner(self, ref_path, vehicle_status):
        ...
        if len(ref_path.poses) > look_distance:
            ...
            try:
                ...
                lane_off_set = [-3.0, -2.5, -2, 2, 2.5, 3.0]
                ...
                for end_point in local_lattice_points:
                    ...
                    for i in waypoints_x:
                        result = a * pow(i, 3) + b * pow(i,
                                                         2) + c * i + d  # 3 차 방정식 수식을 작성한다. (f(x) = a*x^3 + b*x^2 + c*x + d)
