...
class lc_path_pub:
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            if self.is_object_info == True and self.is_odom == True:
                result = self.calc_valid_obj([self.x, self.y, self.vehicle_yaw], self.object_data)
    ...
    def lc_planning(self, global_obj, local_obj, current_waypoint, global_path):
        ...
        '''
        lane_change_distance = 30 * 2  # 60m (point-to-point distance 0.5m)
        ...
        if self.object[0] == True:
            if self.current_lane != 1:
                ...
                end_waypoint_idx = self.get_current_waypoint(
                    self.lc_1.poses[current_waypoint + lane_change_distance].pose.position.x,
                    self.lc_1.poses[current_waypoint + lane_change_distance].pose.position.y, self.lc_1)
                end_point = self.lc_1.poses[end_waypoint_idx]
                start_next_point = self.lc_2.poses[current_waypoint + 5]
                global_path = self.getLaneChangePath(self.lc_2, self.lc_1, start_point, end_point, start_next_point,
                                                     end_waypoint_idx)
                ...
                end_waypoint_idx = self.get_current_waypoint(
                    self.lc_2.poses[current_waypoint + lane_change_distance].pose.position.x,
                    self.lc_2.poses[current_waypoint + lane_change_distance].pose.position.y, self.lc_2)
                end_point = self.lc_2.poses[end_waypoint_idx]
                start_next_point = self.lc_1.poses[current_waypoint + 5]
                global_path = self.getLaneChangePath(self.lc_1, self.lc_2, start_point, end_point, start_next_point,
                                                     end_waypoint_idx)
                self.lane_change = True
                self.object = [False, 0]
    ...
    def check_object(self, ref_path, global_valid_object, local_valid_object):
        ...
        if len(global_valid_object) >0  :
            ...
            for i in range(len(global_valid_object)):
                for path in ref_path.poses :   
                    if global_valid_object[i][0]==1 or global_valid_object[i][0]==2 :  
                        dis = 
                        if dis<2.5:
                            rel_distance=                         
        ...
        if len(global_valid_object) > 0:
            ...
            for i in range(len(global_valid_object)):
                for path in ref_path.poses:
                    if global_valid_object[i][0] == 1 or global_valid_object[i][0] == 2:
                        dis = sqrt(pow(path.pose.position.x - global_valid_object[i][1], 2) + pow(
    ...
    def getLaneChangePath(self, ego_path, lc_path, start_point, end_point, start_next_point, end_waypoint_idx):  ## 
        ...
        start_path_distance = sqrt(pow(start_point.pose.position.x - end_point.pose.position.x, 2) +
                                   pow(start_point.pose.position.y - end_point.pose.position.y, 2))
        ...
        rotation_matrix_1 = np.array([[cos(theta), -sin(theta)],
                                      [sin(theta), cos(theta)]])

        for k in range(0, start_path_repeat + 1):
            rotation_matrix_2 = np.array([[k * point_to_point_distance],
                                          [0]])
            rotation_matrix_calc = np.matmul(rotation_matrix_1, rotation_matrix_2)

            read_pose = PoseStamped()
            read_pose.pose.position.x = start_point.pose.position.x + rotation_matrix_calc[0]
            read_pose.pose.position.y = start_point.pose.position.y + rotation_matrix_calc[1]
