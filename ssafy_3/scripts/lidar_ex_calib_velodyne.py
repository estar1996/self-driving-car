...
def getSensorToVehicleMat(sensorRPY, sensorPosition):
    ...
    sensorTranslationMat = np.array([
        [1, 0, 0, sensorPosition[0]],
        [0, 1, 0, sensorPosition[1]],
        [0, 0, 1, sensorPosition[2]],
        [0, 0, 0, 1]
    ])

    Tr_sensor_to_vehicle = np.array([
        [sensorRotationMat[0][0], sensorRotationMat[0][1], sensorRotationMat[0][2], sensorPosition[0]],
        [sensorRotationMat[1][0], sensorRotationMat[1][1], sensorRotationMat[1][2], sensorPosition[1]],
        [sensorRotationMat[2][0], sensorRotationMat[2][1], sensorRotationMat[2][2], sensorPosition[2]],
        [sensorRotationMat[0][0], sensorRotationMat[0][0], sensorRotationMat[0][0], 1]
    ])
...
def getLiDARTOCameraTransformMat(camRPY, camPosition, lidarRPY, lidarPosition):
    ...
    Tr_vehicle_to_cam = np.linalg.inv(Tr_cam_to_vehicle)
...
def getTransformMat(params_cam, params_lidar):
    ...
    camRPY = np.array([params_cam.get(i) for i in (["ROLL", "PITCH", "YAW"])]) + np.array(
