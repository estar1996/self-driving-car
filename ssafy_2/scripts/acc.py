...
class pure_pursuit :
    ...
    def calc_pure_pursuit(self,):
        ...
        # 
        self.lfd = self.min_lfd + self.lfd_gain * self.status_msg.velocity.x
        ...
        for num,i in enumerate(self.path.poses) :
            ...
            if local_path_point[0]>0 :
                ...
                if dis >= self.lfd :
                    ...
                    break
        '''
        trans_matrix = np.array([[np.cos(self.vehicle_yaw), -np.sin(self.vehicle_yaw), translation[0]],
                                [np.sin(self.vehicle_yaw), np.cos(self.vehicle_yaw), translation[1]],
                                [0, 0, 1]])
        ...
        for num, i in enumerate(self.path.poses):
            ...
            if local_path_point[0] > 0:
                dis = np.sqrt(local_path_point[0]**2 + local_path_point[1]**2)
                if dis >= self.lfd:
        ...
        '''
        if self.is_look_forward_point:
            dx = self.forward_point.x - vehicle_position.x
            dy = self.forward_point.y - vehicle_position.y
            target_angle = np.arctan2(dy, dx)
            theta = target_angle - self.vehicle_yaw
            wheel_base = self.vehicle_length
            if theta > np.pi:
                theta -= 2 * np.pi
            elif theta < -np.pi:
                theta += 2 * np.pi
            steering = np.arctan( 2 * wheel_base * np.sin(theta) / self.lfd)
        else:
            steering = 0
...
class velocityPlanning:
    ...
    def curvedBaseVelocity(self, gloabl_path, point_num):
        ...
        for i in range(point_num, len(gloabl_path.poses) - point_num):
            ...
            A = np.array(x_list)
            B = np.array(y_list)
            circle_params = np.linalg.lstsq(A,B,rcond=None)[0]
            center_x, center_y, c = circle_params
            r = np.sqrt((center_x ** 2) + (center_y ** 2) -c)
            ...
            '''
            gravity = 9.81
            v_max = np.sqrt(r * gravity * self.road_friction)
            ...
            for i in range(len(global_ped_info)):
                for path in ref_path.poses :      
                    if global_ped_info[i][0] == 0 : # type=0 [pedestrian]                    
                        dis = np.sqrt((path.pose.position.x - global_ped_info[i][1])**2 + (path.pose.position.y - global_ped_info[i][2])**2)
                        if dis<2.35:                            
                            rel_distance = np.sqrt((path.pose.position.x - local_npc_info[i][1])**2 + (path.pose.position.y - local_npc_info[i][2])**2)
            ...
            for i in range(len(global_npc_info)):
                for path in ref_path.poses :      
                    if global_npc_info[i][0] == 1 : # type=1 [npc_vehicle] 
                        dis = dis = np.sqrt((path.pose.position.x - global_npc_info[i][1])**2 + (path.pose.position.y - global_npc_info[i][2])**2)
                        if dis<2.35:
                            rel_distance = np.sqrt((path.pose.position.x - local_npc_info[i][1])**2 + (path.pose.position.y - local_npc_info[i][2])**2)     
                            if rel_distance < min_rel_distance:
                                min_rel_distance = rel_distance
            ...
            for i in range(len(global_obs_info)):
                for path in ref_path.poses :      
                    if global_obs_info[i][0] == 2 : # type=1 [obstacle] 
                        dis = np.sqrt((path.pose.position.x - local_obs_info[i][1])**2 + (path.pose.position.y - global_obs_info[i][2])**2)
                        if dis<2.35:
                            rel_distance= np.sqrt((path.pose.position.x - local_obs_info[i][1])**2 + (path.pose.position.y - global_obs_info[i][2])**2)
                            if rel_distance < min_rel_distance:
                                min_rel_distance = rel_distance
                                # self.object=[True,i] 
