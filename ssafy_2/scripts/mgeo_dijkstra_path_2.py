...
class dijkstra_path_pub :
    ...
    def init_callback(self,msg):
        ...
        for from_node_idx, from_node in self.nodes.items():
            node_data = from_node.item_prop()
            now_x = node_data['point']['value'][0]
            now_y = node_data['point']['value'][1]
            
            dist = sqrt(pow(start_x - now_x, 2) + pow(start_y -now_y,2))
            if dist < min_dist:
                min_dist = dist
                min_node_idx = from_node_idx
        

        self.start_node = min_node_idx
        print('start_node: {}'.format(self.start_node))
    ...
    def goal_callback(self,msg):
        ...
        for from_node_id, from_node in self.nodes.items():
            ...
            dist = sqrt(pow(goal_x - now_x, 2) + pow(goal_y -now_y,2))
            if dist < min_dist:
                min_dist = dist
                min_node_idx = from_node_id

        self.end_node = min_node_idx
